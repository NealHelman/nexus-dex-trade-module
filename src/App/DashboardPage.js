    ...
    const loadDashboardData = async () => {
        ...
        try {
            const [balancesData, ordersData, tickerData] = await Promise.all([
                safeApiCall(getAccountBalances, token, secret),
                safeApiCall(getRecentOrders, token, secret, 5),
                safeApiCall(getMarketTicker, 'BTCUSDT')
                    .catch(() => null)
            ]);
            ...
            const processedBalances = balancesData?.data?.data?.list
                ? balancesData?.data?.data?.list
                    .filter(item => {
