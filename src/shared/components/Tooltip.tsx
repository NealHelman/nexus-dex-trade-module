...
import { timing, animations, zIndex } from '../styles.ts';
import { lighten, fade } from '../../utils/color.ts';
...
const TooltipComponent = styled.div<{
    ...
    ({ skin, theme }) => {
        switch (skin) {
            case 'default':
                return {
                    background: lighten(theme.background, 0.2),
...
function TooltipTrigger({
    ...
    const showTooltip = () => {
        ...
        if (onShow) onShow();
        console.log('showTooltip', { rect, tooltipStyles });
    };
